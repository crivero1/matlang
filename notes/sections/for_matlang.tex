...
This is enough since if it is used over vector variables it will allow us to identify when $v=e_1$. Since $S$ and $Z$ give us an order, it is natural to compute one from the other, so
...
\subsubsection{How to get this order with MATLANG with FOR}

The $\for$operator actually iterates over the canonical vector, so it makes sense that we can get this order matrices using this operator. First we compute $$Z_{\text{eq}}=\for v,X. X + f_{>0}\left( \left( \ssum w.Xw+v\right)\cdot v^*\right).$$ Here $f_{>0}$ is the pointwise function that maps an entry to 1 if is greater than zero an to 0 otherwise.
...
Later we will show how to get the identity, and we have that $Z = Z_{eq} - I$, and thus we can compute $S$ and $min(v)$.
...
The second one is similar, but with multiplication: $$\sprod v. E(v)=\for v,X. X\cdot E(v) + min(v)\cdot E(v)$$
...
Note that $$\text{ones}(M)=f_{>0}\left(\ssum v. Mv\right).$$ And $$\text{diag}(u)=\ssum v. (v^*u)\cdot vv^*$$
