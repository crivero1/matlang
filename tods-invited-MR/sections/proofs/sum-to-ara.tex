...
and this also holds when $t(\mathrm{row}_\alpha)\neq t[\gamma_p]=j$.
  ...
  \item If $e(v_1,\ldots,v_k)=(e_1(v_1,\ldots,v_k))^T$ %with $\EDIT{\ttype(e_1)}=(\alpha, \beta)$ then
		\EDIT{then
		$$\arae(e) \coloneqq \rho_{\row\to\col,\col\to\row}(\arae(e_1)).$$
		}
		% \[
		% \arae(e) \coloneqq 
		% \begin{cases}
...
and similarly, $e_2$ by $e_2(v_1,v_2,v_3)=(v_1^T\cdot v_1)\times e_2(v_2,v_3)$, where in addition we replace scalar multiplication with its simulation using $f_{\odot}$ and the ones vector, as mentioned earlier. 
  ...
  \item If $e=e_1\cdot e_2$ with $\EDIT{\ttype(e_1)}=(\alpha, \gamma)$ and $\EDIT{\ttype(e_2)}=(\gamma, \beta)$,
		...
		% If $\gamma\neq 1$ then
		\EDIT{then
			$$\arae (e) \coloneqq  \pi_{\lbrace \row,\col, \gamma_1,\ldots,\gamma_k \rbrace}\left(\rho_{\col\to C}(\arae (e_1))\Join \rho_{\row\to C}(\arae (e_2)) \right),$$
		}
		% $$
		...
		we have
		$$
		\ssem{\rho_{\col\to C}(\arae (e_1))}{\text{Rel}(\I)}(t)=\sem{e}{\I[v_1'\gets b_{i'_1},\ldots,v_\ell'\gets b_{i'_\ell}]}_{i,k}
		$$
		and for $t(C)=k$, $t(\col)=j$, with $t(\gamma''_s)=i''_s$ for $s=1,\ldots, m$
		$$
		\ssem{\rho_{\row\to C}(\arae (e_2))}{\text{Rel}(\I)}(t)=\sem{e}{\I[v_1''\gets b_{i''_1},\ldots,v_m''\gets b_{i''_m}]}_{k,j}.
		$$
