...
Of course, a natural question to ask now is whether this really results in a language suitable for linear algebra? We argue that the correct way to approach this question is to compare our language to arithmetic circuits, \EDIT{which have been shown to capture the vast majority of existing matrix algorithms, from basic ones such as computing the determinant and the inverse, to complex procedures such as Strassen's algorithm (see \cite{ShpilkaY10,allender} for an overview of the area)}.
%, and can therefore be considered to effectively capture linear algebra. 
In the main technical result of this paper, we show that \langfor~\EDIT{restricted to polynomially bounded functions} 
indeed computes the same class of functions over matrices as the ones computed by arithmetic circuit families of \EDIT{polynomial degree}.  
\EDIT{As a consequence, given that circuit families of polynomial degree can compute all linear algebra algorithms mentioned above, \langfor inherits all expressiveness properties of circuits in practice.}

\EDIT{Having established that \langfor provides a good basis for defining linear algebra algorithms}, 
