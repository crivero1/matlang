...
Indeed, using the identifiers of gates, we can regard circuit-related functions as functions $f_n:(\lbrace 0,1 \rbrace^{n})^\ell\rightarrow\lbrace 0,1 \rbrace^n$, in case they return some gate (identifier) or $f_n:(\lbrace 0,1 \rbrace^{n})^\ell\rightarrow\lbrace 0,1 \rbrace$,
...
\EDIT{Let $\mathsf{vec}(w)$ be the $n\times 1$-vector encoding of a word $w\in\{0,1\}^n$}.
...
$e_{\Phi}^{\geq n_0}\coloneqq  e_{\mathsf{min}}^T\cdot e_{\mathsf{iterate}}\cdot e_{\max -2}$ with
    ...
    \EDIT{\begin{align*}\allowdisplaybreaks
        ...
        & \IfThenElse{\mathsf{min}(G_{\mathsf{top}})\cdot \mathsf{min}(V_{\mathsf{top}})}{X_k}{\bigl( \IfThenElse{G_{\mathsf{top}}^T\cdot V_{\mathsf{top}}}{e_{\mathsf{aggregate}}}{e_{\mathsf{extend}}} \bigr)} \\
    \end{align*}
    }
