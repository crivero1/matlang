...
Let $e$ be a \langsum expression. We define a notion of free vector variables, inductively, as follows: $\mathsf{free}(V):=\emptyset$, 
...
\end{itemize}
Clearly, this suffices to show the proposition since expressions $e$ are considered for which $\mathsf{free}(e)=\emptyset$, in which case the above statements reduce to the one given in the proposition.
...
\begin{itemize}
  ...
  \item If $e=v_p$ then $\arae (e):=\rho_{\col_\alpha\to \gamma_p}(R_{v_p})$ if $v_p$ is of type $(\alpha,1)$. It is here that we introduce the attribute $\gamma_p$ associated with iterator variable $v_p$.
 We note that 
$$ \ssem{\arae(v_p)}{\text{Rel}(\I)}(t)=\sem{v_p}{\I[v_p\gets b_{j}]}_{i,1}=(b_j)_{i,1}
$$
for $t(\mathrm{row}_\alpha)=i$ and $t[\gamma_p]=j$ since $(b_j)_{i,1}=\kone$ if $j=i$
and this also holds when $t(\mathrm{row}_\alpha)=t[\gamma_p]=j$, and 
$(b_j)_{i,1}=\kzero$ if $j\neq i$
...
\floris{There is an issue here since $e_1$ and $e_2$ can have different free iterator variables. I think we can ensure that both have the same by introducing them somehow, or alternative, since all operators are linear, by pushing $+$ all the way down? Not sure.}
  ...
  \item If $e=f_\odot(e_1,\ldots, e_k)$ with $\Sch(e_i)=\Sch(e_j)$ for all $i,j\in[1,k]$, then $\arae(e):=\arae(e_1)\Join \ldots \Join\arae(e_k)$.
...
If $\gamma\neq 1$ then
$$
\arae (e) := \pi_{\lbrace \row_{\alpha},\col_{\beta}, \gamma_1,\ldots,\gamma_k \rbrace}\left( \sigma_{\lbrace C_1, C_2 \rbrace} \left(\rho_{\col_\gamma\mapsto C_1}(\arae (e_1))\Join \rho_{\row_\gamma\to C_2}(\arae (e_2)) \right) \right),
$$
when $\text{Rel}(\Sch)(\arae(e_1))=\{\row_\alpha,\col_\gamma,\gamma_1',\ldots,\gamma_{k}'\}$,
$\text{Rel}(\Sch)(\arae(e_2))=\{\row_\gamma,\col_\beta,\gamma_1'',\ldots,\gamma_{\ell}''\}$ and $\{\gamma_1,\ldots,\gamma_k\}=\{\gamma_1',\ldots,\gamma_k',\gamma_1'',\ldots,\gamma_k''\}$.
...
% Hence, by projecting in attributes $\lbrace \row_\alpha, \col_\beta\rbrace$ we range over all $k$ and sum up all $K$-values for each entry.
  ...
  $$
  \arae (e):=\pi_{\text{Rel}(\Sch)(\arae(e_1))\setminus\{\gamma_p\}} \arae (e_2).
  $$
...
$$
\sem{e}{\I[v_1\gets b_{i_1},\ldots,v_{p-1}\gets b_{p-1},v_{p+1}\gets b_{p+1},\ldots,v_k\gets b_k]}_{i,j}.
$$
