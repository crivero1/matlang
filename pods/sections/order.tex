...
\begin{itemize}
	...
	\item $\mathsf{min}(u)$ such that  $\mathsf{min}(b_i^n)=1$ if $i=1$ and $\mathsf{min}(b_i^n)=0$ otherwise; and
	\item $\mathsf{max}(u)$ such that  $\mathsf{max}(b_i^n)=1$ if $i=n$ and $\mathsf{min}(b_i^n)=0$ otherwise.	
	\end{itemize}
...
Having order information available results in \langfor to be quite expressive, just like in classical database query languages. In particular, we heavily rely on the availability of order information to compute the inverse of matrices and more generally to simulate low complexity Turing machines and arithmetic circuits.
