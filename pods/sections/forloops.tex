...
$\lang(\Fun)$ is properly subsumed by $\langfor(\Fun).$
\end{proposition} 
...
\begin{lemma}
\label{lm-prod-sum}
Let $f_\odot^k:\RR^k\mapsto \RR$ and $f_\oplus^k:\RR^k\mapsto \RR$ be the product and the sum function with $k$ attributes, respectively. Then it holds that $\langfor(\emptyset) \equiv \langfor(\{f_\odot^k,f_\oplus^k \ | \ k\in \mathbf{N}\})$.
\end{lemma}
