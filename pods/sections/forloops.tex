...
These examples illustrate that we can limit \langfor to consist of the following ``core'' operators: transposition, matrix multiplication and addition, scalar multiplication, and pointwise function applications. More specific, \langfor is defined by the following simplified syntax:
$$
...
\domagoj{Swap this with Floyd-Warshall, Remove Example 3.6 (TC), and push 4cliques to a comment about the separating example from $\lang$.}

As another example, we show that we can compute whether a graph contains a $4\textsf{clique}$ using \langfor.
