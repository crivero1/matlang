...
\domagoj{Maybe add the division corollary; or maybe after the equivalence result.}
...
%THIS IS WHAT I THOUGHT WE WOULD DO; APPENDIX IS A BIT DIFFERENT:
...
Interestingly, one can easily extend the proof of Theorem \ref{th-circuits-ml} to support arithmetic circuits over matrices. Combining the two results gives us the desired equivalence in terms of functions we can compute.
...
\domagoj{Pretty sure this is what we want to show. Need to define the degree of a \langfor\ expression. Now it makes sense to present the undecidability result here.}
