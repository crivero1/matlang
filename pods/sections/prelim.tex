...
For example, take $\I$ such that $\dom(\alpha)=n$ and let $\conc(V)$ be an adjacency matrix $A$ of an undirected graph $G$ consisting of $n$ vertices. Then, $\sem{\mathsf{cwalk}}{\I}$ returns the number of paths of length two in $G$ which start in and end at the same vertex.
\end{example}
