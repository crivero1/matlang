...
As an example \lang\ expression, consider 
$\mathsf{cwalk}:= (\ones(V))^T\cdot f_{\odot}\bigl(V\cdot V, \diag(\ones(V)\bigr)\cdot\ones(V)$ with $f_\odot:\RR^2\to\RR:(x,y)\mapsto x\cdot y$.
...
We denote size symbols by letters $\alpha,\beta,\gamma$. We also assume that $1\in \DD$. For our example expression $\mathsf{cwalk}$ we let $\Sch$  consist of $\Mnam:=\{V\}$ and $\size(V):=(\alpha,\alpha)$.
To help us determine whether a \lang\ expression can always be evaluated, we define the \textit{type} of an expression $e$, with respect to a schema $\Sch$, denoted by $\ttype(e)$ as follows:
\begin{itemize}
...
\end{itemize}
When $\Sch$ is clear from the context we simply write $\type(e)$. We call an expression \textit{well-typed} according to the schema $\Sch$, if it has a defined type. It is readily verified that $\mathsf{cwalk}$ is well-typed and more specifically, $\ttype(\mathsf{cwalk})=(1,1)$.
...
\end{itemize}
This concludes the specification of \lang. For our example expression $\mathsf{cwalk}$,  consider the instance $\I$ such that $\dom(\alpha)=n$ and $\conc(V)$ is an adjacency matrix $A_G$ of an undirected graph $G$ consists of $n$ vertices. Then, $\sem{\mathsf{cwalk}}{\I}$ returns the number of paths of length two in $G$ which start in and end at the same vertices.
