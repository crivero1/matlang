...
\end{itemize}
This concludes the specification of \lang. For our example expression $\mathsf{cwalk}$,  consider the instance $\I$ such that $\dom(\alpha)=n$ and $\conc(V)$ is an adjacency matrix $A$ of an undirected graph $G$ consisting of $n$ vertices. Then, $\sem{\mathsf{cwalk}}{\I}$ returns the number of paths of length two in $G$ which start in and end at the same vertex.
