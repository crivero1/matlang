...
$A_0=\mathbf{0}$ and in consecutive iterations,  $A_j=A_{j-1}+ b_i^T\cdot A\cdot b_j$ if $j>i$ (because $\mathsf{ssucc}(b_i,b_j)=1$ if $j>i$) and $A_j=A_{j-1}$ otherwise (because $\mathsf{ssucc}(b_i,b_j)=0$ for $j\leq i$). It is now clear that the result of this evaluation is the desired column vector.
...
where $f_/:\RR^2\to\RR:(x,y)\mapsto x/y$ is the division function. It should be clear that when $M$ is assigned to $A$ and $y$ to $b_i$, $f_/(\ccol{A}{b_i},-(b_i^T\cdot A\cdot b_i)\cdot \ones(b_i))$ is equal to the vector $c_i$ used in the definition of $T_i$. To perform the reduction steps for all columns, we consider
the expression
$$
...
There exists \langfor$(f_/)$ expressions $e_L(M)$ and $e_U(M)$ such that
...
If row pivoting is needed $A$, i.e., such that $P\cdot A$ is LU-factorizable for a permutation matrix $P$,
...
if and only if $\Fun$ include division $f_/$ and \langfor$(\Fun)$ can express the function
$\mathsf{minnz}(V)$.
 %
