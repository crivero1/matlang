...
To implement the above procedure, for each column $j$ we need to compute the vector $c_j$. We do this in two steps. First, we extract from our input matrix its $j$th column and set all its upper diagonal entries to zero. This can be achieved by the following expression:
...
$A_0=\mathbf{0}$ and $A_i=A_{i-1}+ b_j^T\cdot A\cdot b_i$ if $i>j$ (because $\mathsf{ssucc}(b_j,b_i)=1$ if $i>j$) and $A_i=A_{i-1}$ otherwise (because $\mathsf{ssucc}(b_j,b_i)=0$ for $i\leq j$). 
...
Using $\ccol{M}{y}$, we can now compute $T_i$s by writing:
...
There are \langfor expressions $e_L(M)$ and $e_U(M)$ such that
