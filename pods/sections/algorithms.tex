...
operation: Given a vector $v$, return $\mathsf{minnz}(v)=b_j$ where $j$ is the first entry in $v$ that holds a non-zero value, or returns $\mathsf{minnz}(v)=\mathbf{0}$ if no such non-zero value exists. In fact, the ability to express
...
$L^{-1}\cdot P=\sem{e_P}{\I}$ and $U=\sem{e_U}{\I}$, $L^{-1}\cdot P\cdot A=U$ holds
...
\end{proposition}
As mentioned above, \langfor$(f_/,f_>0)$ suffice to compute LU decomposition with pivoting for arbitrary matrices.
Intuitively, by allowing $f_{>0}$ we introduce a limited form of \texttt{if-then-else} in \langfor, which is needed
to only continue reducing columns when the right pivot has been found.

\floris{So, we cannot get $e_{P}$ and $e_{L}$ separately? Should we comment on the P-completeness result for LU with pivoting?
(Once we know how to interpret this result in our context?)}


\subsection{Determinant and inverse}
Key linear algebra operations include the computation of the determinant and
...
is now readily verified that $U=(I-D^{-1}_U\cdot U')^{-1}\cdot D_U^{-1}$ with $D_U$ the diagonal matrix consisting of the diagonal entries of $U$ and $U'=U-D_U$. Whereas $D_U^{-1}$ can simply be obtained by inverting the (non-zero) elements on the diagonal by means of $f_/$, we observe that $(I-D^{-1}_U\cdot U')^{-1}=\sum_{i=0}^{n}(-D_U^{-1}U')^i$ which is something we can compute in \langfor. As a consequence:
\begin{proposition}\label{prop:inverse}
There is a \langfor$(f_/)$ expression $e_{\mathsf{inv}}(V)$ such
...
\end{proposition}
Again, we generalize this result, by including $f_{>0}$ in \langfor, to arbitrary
...
and inverse of a matrix can already be defined in \langfor$(f_/)$. So instead of using LU decomposition
with pivoting to treat the general matrix case, we use an alternative solution.

In fact, we can use the
ability of \langfor$(f_/)$ to compute the determinant and inverse for LU-factorizable matrices
in combination with Csanky's algorithm~\cite{}.
\floris{This needs to be filled in when this approach is sure to work.}
%
