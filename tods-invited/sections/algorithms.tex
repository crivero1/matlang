...
%We will answer this in the next section by connecting \langfor with 
...
%\smallskip
%\noindent
%\textbf{LU decomposition by Gaussian elimination.} 
...
\domagoj{Please check that it reads OK. I removed quite a bit, and also redid the notation in Proof of 4.3.}
...
%\end{matrix}\right),
%$$
%with $S_i=\mathsf{tr}(A^i)$. 


%More specifically, we rely on Csanky's algorithm for computing the inverse of a matrix~\cite{Csanky76}. This algorithm uses the characteristic polynomial $p_A(x)=\mathsf{det}(xI-A)$ of a matrix. When expanded as a polynomial $p_A(x)=\sum_{i=0}^{n} c_i x^i$, it is known that
%$A^{-1}=\frac{1}{c_n}\sum_{i=0}^{n-1}c_i A^{n-1-i},$ if $c_n\neq 0$. It also holds that $c_n=(-1)^n\mathsf{det}(A)$, and if $c_n\neq 0$, then $c_0 = 1$. Furthermore, the coefficients $c_1,\ldots,c_n$ are known to satisfy the system of equations $S\cdot c=s$ given by:
%    $$
...
%    \vdots\\
%    c_n\\
%    \end{matrix}\right)}_{c}=\underbrace{\left(\begin{matrix}
%    S_1\\
%    S_2\\
%    S_3\\
%    \vdots\\
%    S_n\\
%    \end{matrix}\right)}_{s},
%    $$
