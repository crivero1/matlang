...
\cristian{Why the output tape moves only to the right and it is write only? There is a mismatch here, because we say that it is write-only, but in the transition relation we read the symbol in the output tape. As far as I can see, we can make the output read/write and move to the two directions, if we assume that the output is of the same size as the input (which is what we are using). Another possibility is to say that the output is never read. I prefer the first option, because it is easy to extend from what we have and I am sure that it works.}
...
\end{proposition}
\cristian{The list of variables seems not necessary for the statement of the result. Remove if it is possible.}
...
Since we want to simulate $T$ we need to be able to check which 
    ...
    \cristian{This expression is wrong. The $\mathsf{min}$ and $\mathsf{max}$ functions (and all other functions for order) works for canonical vectors. If the input is not a canonical vector, it can output any number (not necessarily $0$ or $1$).} 
    ...
    $$
    \cristian{and here.}
    \cristian{To solve this issue with the borders, the safest solution is to have a special variables for the borders, which is true if, and only if, the head is in the border. This variable will work similar than the vector variables for states.}
