...
Let $\Phi_n$ be a circuit with $n$ input gates. We assume an ordering on the children of gates
 % and that there are only variable input gates or input gates with value $1$ ($1$-gates). Indeed, we can prune away input gates with value $0$ in a preprocessing step.
and store the values of the input gates in an $n\times 1$ vector $A$, where $A[i]$ denotes the value
