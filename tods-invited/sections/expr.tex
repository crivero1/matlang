...
\paragraph{$\rhd$  Large $n$.}
 To simulate the two stacks $\cG$ and $\cV$ we use a matrix $X$ of dimensions $n \times n$, where
    ...
    \end{itemize}
\begin{example}
The matrix $X$ encoding $\cG$ and $\cV$ thus takes the forms
    %
...
$$ \underbrace{\left[\begin{smallmatrix}
   ...
   $$
 when $\getsize{\cG} = \getsize{\cV}$ and  when $\getsize{\cG} = \getsize{\cV}+1$, respectively. Here $\texttt{id}_j$ consist of
